...
export default function Home() {
  ...
  useEffect(() => {
    if (btnClicked) {
      async function mostrardata(url) {
        try {
          let objetoData = await apifp(url);
          ...
          console.log('vUserValue:', vUserValue);
          console.log('vPasswordValue:', vPasswordValue);
          // Verifica si hay algún propietario con el mismo correo electrónico que el valor de vUser
          //let prueba = data.;
          //console.log("es email igual a user?", prueba)
          //const isUserValid = objetoData.array_data.some((data) => (data.email === vUserValue && data.password_user === vPasswordValue));

          const userValidEmail = objetoData.array_data.some((data) => (data.email === vUserValue));
          console.log("validacion email: ",userValidEmail)
          const userValidPassword = objetoData.array_data.some((data) => (data.password_user === vPasswordValue));
          console.log("validacion password: ",userValidPassword)
        ...
        setInfo("Error al obtener usuarios")
        // También puedes registrar el error en la consola
        console.error(error);
        }
      }

      mostrardata("http://localhost:3000/getUsers");
      setBtnClicked(false);
    }
  }, [btnClicked]);
        ...
        <div class="w-full max-w-lg px-10 py-8 mx-auto bg-white border rounded-lg shadow-2xl">
          <div class="max-w-md mx-auto space-y-3">
            ...
            <div class="flex gap-3 pt-3 items-center">
              <button id="btn-login" class="border hover:border-indigo-600 px-4 py-2 rounded-lg shadow ring-1 ring-inset ring-gray-300" onClick={()=> setBtnClicked(true)}>
                Login
              </button>
              <a href="#">Olvido su contraseña</a>
            </div>
            {info && <p className="text-red-500 text-sm mt-3">{info}</p>}
          </div>
        </div>
      </div>
